function all_knockout_results= fixMovieRegionNames_reorganized(patientNum, macro_micro)
...
%     
    ...
    if ~isempty(inds)final_unique_regions
    for ii = 1 :  length(inds)
        i = inds(ii);
    ...
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(8).final_unique_regions{i} = 'LAC';
    end
    end
    ...
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(8).final_microwire_regions{j}  = 'LAC';
    end
    end
    %--
    ...
    %---------------- fixing 452
    inds = find(strcmp(all_knockout_results(10).final_unique_regions, 'LOF'));
    if ~isempty(inds)
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(10).final_unique_regions{i} = 'LVMPFC';
    end
    end

    sig_inds = find(strcmp(all_knockout_results(10).final_microwire_regions, 'LOF'));
    if ~isempty(sig_inds)
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(10).final_microwire_regions{j}  = 'LVMPFC';
    end
    end
    %--
    inds = find(strcmp(all_knockout_results(10).final_unique_regions, 'ROF'));
    if ~isempty(inds)
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(10).final_unique_regions{i} = 'RVMPFC';
    end
    end

    sig_inds = find(strcmp(all_knockout_results(10).final_microwire_regions, 'ROF'));
    if ~isempty(sig_inds)
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(10).final_microwire_regions{j}  = 'RVMPFC';
    end
    end
    %--
    inds = find(strcmp(all_knockout_results(10).final_unique_regions, 'LAH'));
    if ~isempty(inds)
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(10).final_unique_regions{i} = 'LSUB';
    end
    end

    sig_inds = find(strcmp(all_knockout_results(10).final_microwire_regions, 'LAH'));
    if ~isempty(sig_inds)
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(10).final_microwire_regions{j}  = 'LSUB';
    end
    end
    %------
    inds = find(strcmp(all_knockout_results(10).final_unique_regions, 'RMH'));
    if ~isempty(inds)
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(10).final_unique_regions{i} = 'RSUB';
    end
    end

    sig_inds = find(strcmp(all_knockout_results(10).final_microwire_regions, 'RMH'));
    if ~isempty(sig_inds)
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(10).final_microwire_regions{j}  = 'RSUB';
    end
    end

    %--
    inds = find(strcmp(all_knockout_results(10).final_unique_regions, 'RTP'));
    if ~isempty(inds)
    for ii = 1 :  length(inds)
        i = inds(ii);
        all_knockout_results(10).final_unique_regions{i} = 'RO';
    end
    end

    sig_inds = find(strcmp(all_knockout_results(10).final_microwire_regions, 'RTP'));
    if ~isempty(sig_inds)
    for jj = 1 : length(sig_inds)
        j = sig_inds(jj);
        all_knockout_results(10).final_microwire_regions{j}  = 'RO';
    end
    end
end