...
%addpath(genpath('/data/Projects/Movie/Movie_Analysis_Repo/Paper_Decoding/Codes'))
 % for memtest fig
%path_to_processed_data = '/data/Projects/Movie/Movie_Analysis_Repo/CNN_LSTM_result';
%path_to_figures_data = '/data/Projects/Movie/Movie_Analysis_Repo/final_result_outputs';

addpath(genpath('./Codes'));
path_to_processed_data = '/media/yipeng/data/movie_2021/Movie_Analysis/paper_results';
%path_to_figures_data = './final_result_outputs';
path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs_0608';
path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs/'
%path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs_old_feb'
%path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs_reverse'
%path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs_50reverse'
%path_to_figures_data = '/media/yipeng/data/movie_2021/Movie_Analysis/final_result_outputs_fake_label'
model_types = {'CNN_multi_2_KLD', 'knockout_test_CNN_KLD', 'LSTM_multi_2_KLD', 'knockout_test_LSTM_KLD'};
path_to_raw_data = './final_result_outputs/data_paper';
models = {'LSTM', 'CNN', "LSTM_MTL", "LSTM_noMTL", "LSTM_100"};
...
patientNums = {'p_431', 'p_433', 'p_435', 'p_436', 'p_439', 'p_441', 'p_444', 'p_445', 'p_452'}; 
...
if iM == 1
    ...
    nn_structure = imread(fullfile(figures_path, 'LSTM928.png'));
else
    axes('position', panel_pos(6,:).*[.1 .9 9.2 4])
    nn_structure = imread(fullfile(figures_path, 'CNN.png'));
end
...
if iM ==1
    y_pos = [1.55, 1.35, 1.15 .95]+0.05;
else
    y_pos = [1.55, 1.35, 1.15 .95];
end
...
fig2def = stats;
if iM == 1 || iM == 3 || iM == 4
fig2def_random = load(fullfile(path_to_figures_data, "LSTM_shuffle_label", 'figure2def.mat')).stats;
else
fig2def_random = load(fullfile(path_to_figures_data, "CNN_shuffle_label", 'figure2def.mat')).stats;
end
cmp_activation_rand = colormap(othercolor('GnBu7', 40));
cmp_activation_rand = cmp_activation_rand(end:-1:1,:);
...
for iP = 1 : length(patientNums)
    this_patient = patientNums{iP};
    all_character_accuracy_random(iP,:) = eval(strcat('fig2def_random','.',this_patient)).character_acc;
    all_character_f1_random(iP,:) = eval(strcat('fig2def_random','.',this_patient)).character_f1_score;
end
...
shadedErrorBar(1:4, nanmean(all_character_f1_random),nanstd(all_character_f1_random), {'color', cmp_activation_rand(5*(1-1)+1,:)/1.2}, .8)
...
shadedErrorBar(1:4, 100*nanmean(all_character_accuracy_random),nanstd(100*all_character_accuracy_random), {'color', cmp_activation_rand(5*(1-1)+1,:)/1.2}, .8)    
xlim([0 6])
set(gca,'xtick', 1:4)
set(gca, 'xticklabel', [])
xtickangle(20)
...
for char_num = 1 : 4
    if iM ==1 
        ...
        axes('position', panel_pos(25+char_num,:).*[.95 - char_num/50 .1 1.1 2])
    end
...
capstr = [];
savefig(fullfile(figures_path, 'Figure_2_April22_2021'), fig, true, capstr, 600);
...
all_regions_unsorted= all_regions;
all_microwires_unsorted= all_microwires;
...
all_region_loss_norm_unsorted = cat(1, all_region_loss_norm{:});
...
load(fullfile(path_to_figures_data, '/patient_performance_new.mat'));
...
temp_I_st = sort(temp_I)
set(gca,'xtick', temp_I_st)
set(gca, 'xticklabel', unique_regions_mw(temp_I_st)); %, 'fontsize', 7)
xtickangle(45)
...
for char_num = 1 : 4
    ...
    set(gca,'xticklabel', [])
    xlim([1 length(unique_sorted_mw_I)])
    if char_num == 4
...
%     set(gca, 'ytick', 0:1)
    ...
    ylabel(strcat('C. ', num2str(char_num)))
    if char_num == 1; text(2, 0.8, 'KLD loss_{region }- \Sigma KLD Loss_{electrode}');end
    %if char_num == 1; text(2, 1.3, 'KLD loss_{region }- \Sigma KLD Loss_{electrode}');end
    if iM ==1;
        if char_num == 1; ylim([0 0.6]); set(gca,'ytick', 0:0.5:0.6);
            text(-.05, 1.6, 'd', 'fontweight','bold', 'fontsize', 9)
        elseif char_num == 2 | char_num == 4; ylim([0 .6]); set(gca,'ytick', 0:.5:.5);
        elseif char_num == 3; ylim([0 1]); set(gca,'ytick', 0:.5:1);end
    else
        if char_num == 1; ylim([0 1.2]); set(gca,'ytick', 0:1.2:1.2);
            text(-.05, 1.6, 'd', 'fontweight','bold', 'fontsize', 9)
        elseif char_num == 2; ylim([0 0.6]); set(gca,'ytick', 0:.5:.5);
        elseif char_num == 4; ylim([-0.1 .6]); set(gca,'ytick', 0:.5:.5);
        elseif char_num == 3; ylim([-0.1 1.01]); set(gca,'ytick', 0:.8:.8);end
    end
end

% 3b top
% group analysis
axes('position', panel_pos(2,:) .*[1.15 .9 .9 1.4])    
...
for iR = 1 : length(regions_forDisplay)
        ...
        [a,b] = binofit(sum(this_loss(:)>.5),length(this_loss(:)));
        display(strcat('percent above .5 for region', regions_forDisplay{iR}))
        a, b
end
...
text(4, 1.3, '* C.2', 'color', cmp(2,:))
text(7, 1.3, '* C.3', 'color', cmp(3,:))
text(10, 1.3, '* C.4', 'color', cmp(4,:))
...
all_loss_high = {};
all_loss_low = {};
low_performers = patientNums(overall_performance <= nanmean(overall_performance)); %(overall_performance>0)));
high_performers = patientNums(overall_performance > nanmean(overall_performance)); %(overall_performance>0)));
for iR = 1 : length(regions_forDisplay)
    valid = strcmp(all_regions_unsorted, regions_forDisplay{iR}) & ismember(all_regions_ptnum, high_performers);
    this_loss_high = all_region_loss_norm_unsorted(valid,:);
    all_loss_high{iR} = this_loss_high(:);
    B = bar(iR-.15, nanmedian(this_loss_high(:)),'facecolor', cmp(1,:) , 'edgecolor', 'none','barwidth', .3);
    %B = bar(iR-.15, nanmean(this_loss_high(:)),'facecolor', cmp(1,:) , 'edgecolor', 'none', 'barwidth', .3)
    B.FaceAlpha = .5;
    hold on

    valid = strcmp(all_regions_unsorted, regions_forDisplay{iR}) & ismember(all_regions_ptnum, low_performers);
    this_loss_low = all_region_loss_norm_unsorted(valid,:);
    all_loss_low{iR} = this_loss_low(:);
    B = bar(iR+.15, nanmedian(this_loss_low(:)),'facecolor', cmp(3,:) , 'edgecolor', 'none', 'barwidth', .3);
    %B = bar(iR+.15, nanmean(this_loss_low(:)),'facecolor', cmp(3,:) , 'edgecolor', 'none', 'barwidth', .3);
    B.FaceAlpha = .5;
...
[corrected_p_low_high, h]=bonf_holm(p_low_high,0.1);
% grouping regions together
Frontal_loss_high = [all_loss_high{1};all_loss_high{5}; all_loss_high{11}];
MTL_loss_high = [all_loss_high{3}; all_loss_high{4}];
association_loss_high = [all_loss_high{6};all_loss_high{7}; all_loss_high{8}; all_loss_high{10}];


Frontal_loss_low = [all_loss_low{1};all_loss_low{5}; all_loss_low{11}];
MTL_loss_low = [all_loss_low{3}; all_loss_low{4}];
association_loss_low = [all_loss_low{6};all_loss_low{7}; all_loss_low{8}; all_loss_low{10}];
...
if iM == 1
    ...
    text(.5,.22,'High performer', 'color', cmp(1,:),  'fontweight','bold', 'fontsize', 7)
    text(.5,.19,'Low performer', 'color', cmp(3,:), 'fontweight','bold', 'fontsize', 7)
end
%}
axes('position', panel_pos(6,:) .*[1.15 1.35 .9 1.75])    
...
for iR = 1 : length(regions_forDisplay)
    ...
    loss_diff(iR, :) = n;
    p = signrank(this_loss(:));
    p_whole = [p_whole;p];
    median_whole = [median_whole; nanmean(this_loss(:))];
end
...
xtickangle(25)
ylim([-.5 .5])
set(gca,'ytick', -.5:.25:.5, 'yticklabel',{'-0.5', '', '0', '', '0.5'})
caxis([0 6])

for iR = 1 : length(regions_forDisplay)
    if p_whole(iR)<.025
        plot([iR-.5, iR+.5], [median_whole(iR), median_whole(iR)], 'r')
    end
    if p_whole(iR)<.025 & p_whole(iR)>.01
        text(iR-.5,-.4,'*','fontsize', 10)
        elseif p_whole(iR)<.01 & p_whole(iR)>.001
        text(iR-.5,-.4,'**','fontsize', 10)
        elseif p_whole(iR)<.001
        text(iR-.5,-.4,'***','fontsize', 10)
    end
end
...
capstr = [];
savefig(fullfile(figures_path, 'Figure_3_April22_2021'), fig, true, capstr, 600);
...
memory_patients = {'431', '435', '436', '441'};
all_memory_results = [];
for iP = 1:length(memory_patients)  
    ...
    for iF = 1 : length(folds)
     ...
     trigs = load(fullfile(path_to_raw_data,...
         strcat(memory_patients{iP}, '/', memory_patients{iP}, 'memtest1/psychophysics/EXP_',memory_patients{iP}, 'memtest1.mat')));
    data.trigs = trigs;
...
% Qiujing's changes here for panel b:
...
axes('position', panel_pos(2,:).*[1 1 .4 1])
i = 1;
...
axes('position', panel_pos(2,:).*[1.35 1 .4 1])
...
cb.Position(4) = cb.Position(4) * .75;
cb.Position(2) = cb.Position(2) - .09;
cb.Position(1) = cb.Position(1) - .02;
cb.Ticks = 0:1;
...
% of the character
% Qiujing's change here
tmp_val = eval(strcat('fig4b_percentage.p_', memory_patients{ip}));
patient_memory_percentange = tmp_val(:,1);
...
box off
xlabel({'Character size','% in frame'})
...
box off
xlabel({'Character presence','% in clip'})
ylim([.98 1.1])
...
%xlim([-.25 1])
text( 0.05, text_y-text_y_step, 'Character in & Seen', 'color', cmp_activation(10*(1-1)+1,:)/1.5)
text( 0.05, text_y-2*text_y_step, 'Character in & not Seen', 'color', cmp_activation(10*(2-1)+1,:)/1.5)
text( 0.05, text_y-3*text_y_step, 'Character out & Seen', 'color', cmp_activation(10*(3-1)+1,:)/1.5)
text( 0.05, text_y-4*text_y_step, 'Character out &  not Seen', 'color', cmp_activation(10*(4-1)+1,:)/1.5)
xlim([0 1])
box off
% Qiujing is changing here:
%ylim([0.08 .16])
set(gca,'ytick', 0:.08:.16, 'yticklabel', 0:.5:1)

%ylim([0 0.3])
%set(gca,'ytick', 0:.15:.3, 'yticklabel', 0:.5:1)
...
[rho_1, p_1] = corr(first_step_all', memory_viewing(pt_i,:)','type', 'spearman');


[rho_2, p_2] = corr(first_step_all', memory_response(pt_i,:)', 'type', 'spearman');
[rho_3, p_3] = corr(second_step_all', memory_response(pt_i,:)', 'type', 'spearman');
...
%%% added %%%
all_res_cond = [];
index = [[2:5], [7:10], [12:15]];
for pt_i = 1:4
x = first_step_all(index)';
y = memory_viewing(pt_i,index)';
...
signrank(memory_response_trimmed(:)- memory_viewing_trimmed(:))
%xlim([-.25 .25])
xlim([-.25 .40])
box off
...
%savefig(fullfile(figures_path, 'Figure_4_March6_2021'), fig, true, capstr, 600);
savefig(fullfile(figures_path, 'Figure_4_July20_2021'), fig, true, capstr, 600);
%%
...
%this_model  = model_types{3}; % LSTM
if iM ==1
    this_model  = model_types{3};
else
    this_model  = model_types{1};
end
