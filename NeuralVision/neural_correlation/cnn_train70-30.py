...
def multiple_train(patientNums = patientNums, mask=None, project_dir = project_dir ):
    for this_patient in patientNums:
        if zero_signal:
            #path_to_model = os.path.join(project_dir, 'CNN_result_zero',model_option+'_multi_'+str(feature_time_width) +'_KLD')
...
def multiple_erasing(patientNums = patientNums):
    for this_patient in patientNums:
        path_to_model = os.path.join(project_dir, 'CNN_result',model_option+'_erasing_multi_check1_'+str(feature_time_width))
        if not os.path.exists(path_to_model):
            os.mkdir(path_to_model)
