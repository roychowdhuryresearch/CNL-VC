...
def standard_train(patientNums = patientNums):
    path_to_model = os.path.join(project_dir, 'CNN_result','LSTM_Donot_touch')
    #path_to_model = os.path.join(project_dir, 'CNN_result','CNN_result')
...
def standard_test(patientNums = patientNums):
    path_to_model = os.path.join(project_dir, 'CNN_result_2_epoch','basic')
    for this_patient in patientNums:
        train_cnn_model_results(this_patient, masked=None, erasing = False, path_to_model = path_to_model, mode = "test")

def erasing_region(this_patient, dirct = os.path.join(project_dir,'CNN_result'), kfold = 5):
    path_to_model_dir = os.path.join(dirct,model_option+'_retrain')
    ...
    for d in region:
        reg.append(d[0])
    reg_single = sorted(removeDuplicates(reg))
    print(reg_single)
    reg = np.array(reg) 
    for r_idx in range(len(reg_single)):
        r = reg_single[r_idx]
        ...
        '''
        print(mask)
        mask2 = np.where(reg == reg_single[r_idx + 1])[0]
        print(mask2)
        mask3 = np.where(reg == reg_single[r_idx + 2])[0]
        print(mask3)
        mask4 = np.where(reg == reg_single[r_idx + 3])[0]
        print(mask4)
        mask5 = np.where(reg == reg_single[r_idx + 4])[0]
        print(mask5)
        mask6 = np.where(reg == reg_single[r_idx + 5])[0]
        print(mask6)
        mask7 = np.where(reg == reg_single[r_idx + 6])[0]
        print(mask7)
        mask8 = np.where(reg == reg_single[r_idx + 7])[0]
        print(mask8)
        mask9 = np.where(reg == reg_single[r_idx + 8])[0]
        print(mask9)
        mask = np.concatenate((mask, mask2, mask3, mask4, mask5,mask6, mask7, mask8))
        '''
        train_cnn_model_results(this_patient, masked=mask, erasing = True, path_to_model = path_to_model, mode = "train", kfold=kfold)

def erasing_multiple_region(this_patient, dirct = os.path.join(project_dir,'CNN_result'), kfold = 5):
    path_to_model_dir = os.path.join(dirct,model_option+'_retrain')
    if not os.path.exists(path_to_model_dir):
        os.mkdir(path_to_model_dir)
    path_to_model_dir = os.path.join(path_to_model_dir, this_patient)
    ...
    path_to_model = os.path.join(path_to_model_dir, "_".join(np.array(reg_single)[np.array(reg_index)]))
    print(path_to_model)
    if not os.path.exists(path_to_model):
        os.mkdir(path_to_model)
    train_cnn_model_results(this_patient, masked=masks, erasing = True, path_to_model = path_to_model, mode = "train", kfold=kfold)

def keep_region(this_patient, dirct = os.path.join(project_dir,'CNN_result'), kfold = 1):
    path_to_model_dir = os.path.join(dirct,'keep_important')
    if not os.path.exists(path_to_model_dir):
        os.mkdir(path_to_model_dir)
    path_to_model_dir = os.path.join(path_to_model_dir, this_patient)
    if not os.path.exists(path_to_model_dir):
        os.mkdir(path_to_model_dir)
    #region_fn = os.path.join(path_to_training_data,this_patient,"features_mat_regions_clean.npy")
    region_fn = os.path.join(path_to_matlab_generated_movie_data,this_patient,"features_mat_regions_clean.npy")
    region = np.load(region_fn,allow_pickle=True)
    reg = []
    for d in region:
    ...
    reg = np.array(reg) 
    mask = np.where(reg == reg_single[2])[0]
    train_cnn_model_results(this_patient, masked=mask, erasing = False, path_to_model = path_to_model_dir, mode = "train", kfold=kfold)

def multiple_train_p(patientNums = patientNums, mask=None, project_dir = project_dir ):
    """train with fixed 10% validation data but with varying training data size
    p_val = 0.3 means using 50% of training data in the kfold to train
    p_val = 0.2 means using 60% of training data in the kfold to train
    """
    for this_patient in patientNums:
        path_to_model = os.path.join(project_dir, 'CNN_result','CNN_trainp0.6_'+str(feature_time_width))
        if not os.path.exists(path_to_model):
            os.mkdir(path_to_model)
        #train_cnn_model_results(this_patient, masked=None, erasing = False, path_to_model = path_to_model, mode = "train", kfold=5, p_val=0.5)
        train_cnn_model_results(this_patient, masked=None, erasing = False, path_to_model = path_to_model, mode = "train", kfold=5, p_val=0.4)

def multiple_train(patientNums = patientNums, mask=None, project_dir = project_dir ):
    for this_patient in patientNums:
        if zero_signal:
            if model_option is "LSTM":
                path_to_model = os.path.join(project_dir, 'CNN_result',model_option+'_multi_'+str(feature_time_width) +'_KLD_fake_label')
                #path_to_model = os.path.join(project_dir, 'CNN_result',model_option+'_multi_'+str(feature_time_width) +'_KLD_random_label')
            if model_option is "CNN":
                path_to_model = os.path.join(project_dir, 'CNN_result', model_option+'_multi_'+str(feature_time_width) +'_KLD')
                #path_to_model = os.path.join(project_dir, 'CNN_result', model_option+'_multi_'+str(feature_time_width) +'_KLD_random_label')
        else:
...
def multiple_erasing(patientNums = patientNums):
    for this_patient in patientNums:
        path_to_model = os.path.join(project_dir, 'CNN_result',model_option+'_erasing_hemisphere_'+str(feature_time_width))
        if not os.path.exists(path_to_model):
            os.mkdir(path_to_model)
        #erasing_region(this_patient,path_to_model ,kfold=5)
        erasing_multiple_region(this_patient,path_to_model ,kfold=5)

def multiple_keep(patientNums = patientNums):
    for this_patient in patientNums:
        path_to_model = os.path.join(project_dir, 'CNN_result','CNN_keep_multi_'+str(feature_time_width))
        if not os.path.exists(path_to_model):
            os.mkdir(path_to_model)
        keep_region(this_patient,path_to_model ,kfold=5)
...
def standard_train_region_tag(patientNums = patientNums):
    path_to_model = os.path.join(project_dir, 'CNN_result','region_tag_noweight')
    for this_patient in patientNums:
...
def standard_train_region_tag_70(patientNums = patientNums):
    path_to_model = os.path.join(project_dir, 'CNN_result','repeat_test_rg')
    for this_patient in patientNums:
        train_cnn_model_results(this_patient,masked=None, erasing = False, path_to_model = path_to_model, mode = "train")
