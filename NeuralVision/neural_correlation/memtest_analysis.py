...
#result_dir = "/media/yipeng/toshiba/movie/Movie_Analysis/CNN_result_zero/CNN_multi_2_KLD"
#result_dir = "/media/yipeng/toshiba/movie/Movie_Analysis/CNN_result/LSTM_zero"
...
def get_result_board(exp_file, triggle_file):
    ...
    for i in range(exp_file.shape[0]):
        ...
        time_res = (np.array([triggle_file_start[i], triggle_file_end[i]+response_time[i]])*7.5 - offset * 7.5 -start_sample_idx).astype(int) 
...
def parse_interval_appearance(result_board, label_board, patient_prob):
    interval_list = []
    for ep_number in [1, 2]:
        ep_location = np.where(label_board == ep_number)[0]
        ranges = list(find_ranges(ep_location))    
        for r in ranges:
            interval = Interval(r[0], r[1])
            interval.set_episode(ep_number)
            label = result_board[:,r[0]:r[1]]
            interval.set_cv_label(label)
            prediction = []
            for character_index in patient_prob.keys():
                prediction.append(patient_prob[character_index][r[0]:r[1]])
    ...
    for patient in patientNums:
        ...
        for folder in file_list:
            if "memtest1" in folder:
                ...
                end_time_stamp = triggle_file_end[-1] 
                offset = 2520
                start_sample_idx = int((start_time_stamp - ts_offset - offset)*7.5)
                start_sample_idx = int((start_time_stamp - ts_offset - offset)*7.5 + 0.9999/7.5)
                end_sample_idx = int((end_time_stamp - ts_offset - offset)*7.5) +1
                print("start, end_sample idx:", (start_time_stamp- ts_offset - offset)*7.5, (end_time_stamp - ts_offset - offset)*7.5)
                result_board, label_board = get_result_board(exp_file, triggle_file)
        ...
        #if True:
        #    patient_result_folder = os.path.join(result_dir, patient)
            ...
            plot_prob_hist_all(patient_prob_cut, label_board, patient_result_folder)
            plot_prob_hist_char(patient_prob_cut,result_board,label_board,patient_result_folder)
            interval_list = parse_interval_appearance(result_board, label_board, patient_prob_cut)
            #for i in interval_list:
            #    print(i)
            plot_interval_prob(interval_list, patient_result_folder)
            plot_interval_label(interval_list, patient_result_folder)
